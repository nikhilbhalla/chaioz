...
export default function MenuItemEditor({ open, onClose, item, onSaved }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="bg-white border-chaioz-line text-chaioz-teal max-w-2xl" data-testid="menu-editor-dialog">
        <DialogHeader>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <Input value={form.name} onChange={set("name")} data-testid="menu-name" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1" />
              </div>
              <div>
                <Label className="text-chaioz-teal/80">Price ($)</Label>
                <Input type="number" step="0.01" value={form.price} onChange={set("price")} data-testid="menu-price" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1" />
              </div>
              <div>
                <Label className="text-chaioz-teal/80">Category</Label>
                <Select value={form.category} onValueChange={(v) => setForm((f) => ({ ...f, category: v }))}>
                  <SelectTrigger data-testid="menu-category" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1"><SelectValue/></SelectTrigger>
              ...
              <div>
                <Label className="text-chaioz-teal/80">Subcategory</Label>
                <Input value={form.subcategory || ""} onChange={set("subcategory")} data-testid="menu-subcategory" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1" />
              </div>
              <div>
                <Label className="text-chaioz-teal/80">Calories</Label>
                <Input value={form.calories || ""} onChange={set("calories")} data-testid="menu-calories" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1" placeholder="e.g. 420 kcal" />
              </div>
            </div>
            <div>
              <Label className="text-chaioz-teal/80">Description</Label>
              <Textarea value={form.description} onChange={set("description")} data-testid="menu-description" className="bg-chaioz-cream border-chaioz-line text-chaioz-teal mt-1" />
            </div>
        ...
        <div className="flex justify-end gap-2 pt-4 border-t border-chaioz-line">
          <Button variant="outline" onClick={onClose} data-testid="menu-cancel" className="rounded-full bg-transparent border-chaioz-line text-chaioz-teal hover:text-chaioz-saffron">Cancel</Button>
          <Button onClick={save} disabled={saving} data-testid="menu-save" className="rounded-full bg-chaioz-saffron text-chaioz-teal hover:bg-chaioz-saffronHover hover:text-chaioz-teal">
